    ...
    public static class MingFateCausalityPath
    {
        ...
        private static CultivationPathDef Build()
        {
            ...
            var curve = new RealmCurveDef(
                new[] { 16, 20, 26, 40, 57, 88, 141, 231, 385, 660 }, // INV-CROSS v2: buff +10~12% UT4+ (module-weak flag); UT8=1.91x sword (was 1.74x)
                // A1.2 迁移（境界稿 §11.1 重灾区）：低段全奇数 UT1/3/5/7 各压相邻偶数主阶
                //   （1→0,3→2,5→4,7→6）→ 全落锚集 {0,2,4,6,8,9,10,11,12}、非降；
                //   命修「推衍阶细」密度走 SubLevel 不强充独立 UT（决策③）。
                ...
                // —— A.1：SubLevelCount = 同 UT 段长（UT0 段 2 = 观星/知命 → 大境界数 9）；
                //    CanAscend=true；MaxMajor=大境界数-1=8。——

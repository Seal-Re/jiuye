    ...
    public static class FuXiuFuluPath
    {
        ...
        private static CultivationPathDef Build()
        {
            ...
            var curve = new RealmCurveDef(
                new[] { 10, 14, 19, 25, 33, 43, 56, 73 },
                new[] { 0, 1, 2, 3, 4, 6, 8, 10 },
                new[] { "画符学徒", "制符入门", "本命符成", "符箓通玄", "符海大成", "万符朝宗", "符道宗师", "纸笔通神·万象归符" },
                // 制符里程累进阈值 = Σ 100×(0..i-1)（前期投资型,与剑修同构,realm0=0 起底）。
                new[] { 0, 100, 300, 600, 1000, 1500, 2100, 2800 },
                // —— A.1 境界稿 §2：起步 SubLevelCount 全 1（符 UT1/3 偏离 + 顶补 UT11/12 属 A1.2）；
                //    CanAscend=true（符修是修士）；MaxMajor=大境界数-1。——

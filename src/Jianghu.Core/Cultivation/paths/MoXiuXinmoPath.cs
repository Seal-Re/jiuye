...
namespace Jianghu.Cultivation
{
    ...
    public static class MoXiuXinmoPath
    {
        ...
        private static CultivationPathDef Build()
        {
            ...
            //    境界名（炼魔→凝魔→魔丹→魔婴→化魔→魔虚→魔合→魔尊，纯心魔轨；UT0/2/4/6/8/9/10/11 对齐 backbone 阶，A1.3 修原 backbone 名错位）/ 升入阈值（夺养里程累进，realm0=0 起）。——
            var curve = new RealmCurveDef(
                new[] { 10, 16, 20, 34, 55, 89, 139, 216 }, // INV-CROSS v2: nerf -22~24% UT4+; UT8=1.20x sword (was 1.57x)
                new[] { 0, 2, 4, 6, 8, 9, 10, 11 },
                new[] { "炼魔", "凝魔", "魔丹", "魔婴", "化魔", "魔虚", "魔合", "魔尊" },

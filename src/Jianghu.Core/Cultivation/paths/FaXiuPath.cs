    ...
    public static class FaXiuPath
    {
        ...
        private static CultivationPathDef Build()
        {
            ...
            var curve = new RealmCurveDef(
                new[] { 10, 14, 20, 29, 45, 67, 103, 162, 255 }, // INV-CROSS校准: 对齐剑修target曲线
            ...
            var skills = new[]
            {
                ...
                new CombatSkillDef("sk_fa_wulei", "五雷正法", 5,
                    new[]
                    {
                        ...
                        Modules.CounterMul("evil", 3, 2, "对tag:evil(阴邪:鬼/魔/血/妖/蛊)破阴邪×3/2(联合上界)"),
                    },
                ...
                new CombatSkillDef("sk_fa_wanjian", "万剑诀·御物千刃", 4,
                    new[]
                    {
                        Modules.FlatPen(12, "御使飞剑/法宝群攻基线12"),
                        Modules.PenFromResource("spellBreadth", 2, note: "伤害+2×spellBreadth(法术库广度转伤)"),
                    },
                    new Dictionary<string, int> { { "manaPool", 15 } }),
                // 寒冰六合困：范围冻结,敌全体可释战技数-1共1回合。manaPool≥18,消耗18。
                new CombatSkillDef("sk_fa_hanbing", "寒冰六合困", 4,
                    new[] { Modules.FlatPen(10, "范围冻结,敌全体可释战技数-1共1回合(控场)") },
                ...
                new CombatSkillDef("sk_fa_dadun", "五行大遁术", 3,
                    new[]
                    {
                        Modules.FlatPen(0, "强行脱战/贴近,规避一次必杀"),
                ...
                new CombatSkillDef("sk_fa_fuyin_dun", "符印护体·闪", 2,
                    new[] { Modules.Evade(30, "符印护体闪避:30%来袭减免(需符印→门控)") },
                    new Dictionary<string, int> { { "manaPool", 6 } }),
            };

...
**九野**是一个武侠江湖的**涌现模拟内核**——程序化生成武侠角色，让角色在开放式连续时间中按各自节奏自主行动，产出可读的"江湖编年史"。Core 为纯 C# 逻辑库（netstandard2.1），CLI 驱动；后期 Godot 4.x (.NET) 直接引用 Core 作为渲染/玩家介入宿主（引擎目标 2026-07-03 由 Unity 切换，[adr-0004](../../docs/architecture/adr-0004-godot-view-host-boundary.md)）。
...
**后期（Godot 玩家介入）**：玩家操控角色，出招是回合策略（选招/调度资源），关键时刻的闪/格/打断是即时按键——回合谋略 + 即时反应的复合快感。
...
| Godot 宿主（后期） | 依赖 Jianghu.Core 库（零改写，Godot 4.x .NET 直接引用 netstandard2.1） |

...
- [x] **2.1 Godot 项目骨架**：Godot 4.x .NET 项目可编译，引用 `Jianghu.Core`（netstandard2.1）成功
- [x] **2.2 WorldBridge 节点**：`WorldBridge : Node` 挂场景，订阅 Core 事件（至少 `World.Advance` 后的 snapshot）
- [x] **2.3 Signal 广播**：Core `DomainEvent` → `WorldBridge` → Godot `[Signal]` → 控制台 `GD.Print`
- [x] **2.4 最小闭环**：`dotnet run` CLI 同 seed 跑 N 步 → Godot 宿主同样 seed 跑 N 步 → 控制台输出相同事件序列
- [x] **2.5 B.2/B.3 守**：Core 不改一行；`Godot.*` 仅出现在 Godot 宿主程序集
...
**Status**: [x] 已实现 — 2026-07-17，1271 绿；Godot headless snapshot vs CLI 逐字节一致

...
- [ ] **3.1 累加器实现**：Godot 宿主侧 `_Process(delta)` 累加 `_accumulator += delta`；`while (_accumulator >= SimStepSeconds) { World.Advance(1); _accumulator -= SimStepSeconds; }`
- [ ] **3.2 插值因子**：渲染帧读取 `_accumulator / SimStepSeconds` 作为插值因子（0→1），但不影响 Core 状态
- [ ] **3.3 追帧上限**：设每帧最大追帧数（防掉帧螺旋）；默认 `MaxCatchupSteps = 5`
- [ ] **3.4 delta 不进 Core**：`World.Advance` 参数不含 `delta`；`SimStepSeconds` 是 Core 侧常量（整数毫秒 or 来自 `LimitsConfig`）
- [ ] **3.5 确定性验证**：同 seed，不同帧率（60fps vs 10fps）→ 同 `Advance` 步数 → 同轨迹（World state 逐字节一致）

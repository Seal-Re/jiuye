...
- [ ] 9.1 `CombatExchangeResult` sealed record：DmgToDefender / ReflectToAttacker / PoiseBreakBonus / ChipImmuneToPoise / FrameHook
- [ ] 9.2 `ResolveExchange` 返回 `CombatExchangeResult`（替换 5 元组）
- [ ] 9.3 所有调用方适配（ResolveR2 ×2 + 全量测试文件）
- [ ] 9.4 行为零改变——全量 1271 绿不退
- [ ] 9.5 B.2/B.3 守（IL 浮点零 + off 逐字节）

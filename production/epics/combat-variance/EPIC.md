...
**Layer**: Core（战斗内核范式重构）
**Status**: In Progress（cv-001 ready-for-dev；adr-0008 Accepted @ 2026-07-06 落地起点）
...
- [ ] cv-001 主动交锋概率化：CombatMath 查表 + Duel=9 流 + ResolveExchange 伯努利拦截，off 逐字节 + IL 浮点零 + 同种子复现
- [ ] cv-002 削韧/硬直副轴（duel-local，接 balance-007/008）
- [ ] cv-003 标签门控 + Chip Damage
...
- **story-001** active-clash-variance — **Ready**（本 epic 首切片；详文已展开）。CombatMath 查表 + RngStreamIds.Duel=9 + ResolveExchange 插入伯努利判定 + duel-local seed 派生。后台 NPC 内核代掷，先让 `dotnet run` 见"差999"变有悬念对局。
- **story-002** poise-stagger-subaxis — Backlog（详文待 cv-001 done 后展开）。削韧副轴 + 硬直清队列 + 接 balance-007/008 DR + 连续格挡防守帧递减（adr-0008 决策⑦步7/⑧/⑩.3）。duel-local Poise，不入 Clone。

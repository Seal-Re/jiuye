...
- [x] 1.1 21 路 daoHeart_init 乘子表落地（default *2, soul/buddhist *3, body/ghost *2, 新增 9 路各有初值）
- [x] 1.2 每路 ≥3 个 daoHeart 增益来源（事件/动作→道心增量，整数）
- [x] 1.3 每路 ≥3 个 innerDemon 来源（事件/动作→心魔增量，整数）
- [x] 1.4 道心表数据驱动——加路=加数据行，不改 CultivationState/引擎
- [x] 1.5 佛修（buddhist_golden_body）破戒规则初值：vow 折半非归零
- [x] 1.6 全路 standalone 测试（每路 Def 可独立验证 daoHeart_init > 0）
- [x] 1.7 off 模式：道心表存在但 CultivationState.daoHeart=0（不初始化，off 守）
...
**Criteria**: 7/7 AC passed
**Implementation**: `DaoHeartDef.cs` (data model) + `DaoHeartRegistry.cs` (21-path table with gains/demons) + `DaoHeartTableTests.cs` (32 tests)
**Test Evidence**: 594 green (562 → +32). 21 standalone + 3 gain/demon count + 2 registry + 1 buddhist vow + 3 data validation + 1 off mode + 1 aux check.
**Commits**: pending

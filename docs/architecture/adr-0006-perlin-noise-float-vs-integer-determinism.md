...
- **Date**: 2026-07-04（自 godot-architecture-manifest.md §4 落盘引出，architecture.md §10.2 R2 升格）
...
> **DEFERRED（未裁决）。** 先答前置问题："柏林地形结果是否入 Core 确定性路径（A* 权重 / 属性乘区）？" 再选下列备选。
...
- [ ] 用户答前置问题：地形是否入 Core 确定性路径
- [ ] 选定备选写入本 ADR（Status → Accepted）
- [ ] （PCG 未来）若地形入 Core：`ILFloatScanner` 绿（Core 零浮点）；同种子地形网格逐字节一致
- [ ] （PCG 未来）若噪声属 View：确认地形值不流入任何 `Jianghu.Cultivation` 结算

...
### 现状（当前无瓶颈实证）
- **模拟规模由 `LimitsConfig` 约束**（角色数/节点数上限，F-PERF-2）——当前**非海量**（headless 模拟，1062 测试 10s 跑完，active.md）。
...
- **Cons**：**颠覆性重构**——确定性子流绑定、`Clone` 深拷语义、off 逐字节 record、侧表纪律全需重新设计验证；1062 测试大面积重写；高风险（一处确定性 bug 难查，红线 B.7 高风险档）。
...
### Neutral
- 当前 OOP 架构完全可运转（1062 绿），本 ADR 不改任何现状。

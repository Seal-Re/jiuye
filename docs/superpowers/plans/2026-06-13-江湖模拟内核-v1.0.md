...
> **POST-REVIEW FIX (opus 复审)**：下方 double 版违反 spec §4.1（确定性路径禁浮点——IL2CPP/FMA 1-ULP 漂移会让同种子在 Unity 产出不同角色，破坏跨运行时复现），且有 `total==0`→NaN→IndexOutOfRange 的理论崩溃路径。**实际实现改为纯整数定点最大余数法**（sum of `NextUInt` 整数权重 + `budget*w/total` 整数除/取余 + `total==0` 退化保护），分布形状（偏中庸）与 sum/cap 不变量不变。分布测试同步收紧为多种子×全 4 维、5x bar。以提交的 `StatGenerator.cs` 为准。
...
namespace Jianghu.Stats
{
    /// <summary>偏中庸、定和+封顶、无拒绝采样（§5.2）。注：实际实现为纯整数版（见上方 POST-REVIEW FIX）。</summary>
